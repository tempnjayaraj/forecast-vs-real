...
function Forecast() {
  ...
  for (let i = 0; i < 12; i++) {
    ...
    if (j - 11 + i > 11) {
      ...
      displayArray.push(
        <th>{monthNames[j - 11 + i - 12] + "-" + Number(thisYear + 1)}</th>
      );
    } else if (j - 11 + i < 0) {
      let month = j - 11 + i + 11;
      let year = thisYear - 1;
      ...
      let year = thisYear;
      columns.push(month + "" + year);
      displayArray.push(<th>{monthNames[j - 11 + i] + "-" + thisYear}</th>);
    }
  }
  ...
  function submit() {
    // toDBFC.isInFlow = document.getElementById('flow').value=='in' ? true : false;
    toDBFC.cfo_id = document.getElementById("object").value;
    toDBFC.year = document.getElementById("year").value;
    let elementArray = [];
    for (let j = 0; j < 12; j++) {
      singleEntryFC.amount = document.getElementById("amount" + j).value;
      singleEntryFC.gst = document.getElementById("gst" + j).value;
      singleEntryFC.tds = document.getElementById("tds" + j).value;
      elementArray.push(JSON.parse(JSON.stringify(singleEntryFC)));
    }
    toDBFC.data = elementArray;
    let okay = postJson("http://localhost:8080/postForeCastData", toDBFC);
